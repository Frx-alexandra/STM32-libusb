...
int main()
{
	...
	if (find_my_device(0x1a15, 0x462))
	{
		while (true)
		{
			std::cout << "输入命令：";
			...
			if (op[0] == 'w')
			{
				...
				count = usb_bulk_write(my_device, EP_OUT, (char *)writeByte, 11, 0);
				std::cout << "写入数据量:" << count << endl << endl;
			}
			else if (op[0] == 'r')
			{
				count = usb_bulk_read(my_device, EP_IN, receiveByte, 512, 100);
			...
			else if (op[0] == 't')
			{
				...
				for (int i = 0; i < 2048; i++)
				{
					...
					count = usb_bulk_write(my_device, EP_OUT, (char *)writeByte, 512, 0);
					if (count != 512)
					{
						std::cout << "写入错误！" << endl;
						break;
					...
					count = usb_bulk_read(my_device, EP_IN, receiveByte, 512, 500);
					if (count != 512)
					{
